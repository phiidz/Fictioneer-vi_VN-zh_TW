...
require_once __DIR__ . '/_register-meta-fields.php';
 ...
 * @since 5.34.0 - Merged most stylesheet back together.
 */
...
function fictioneer_style_queue() {
  ...
  if ( ! get_option( 'fictioneer_disable_properties' ) ) {
    $main_file_name .= '-properties';
  }

  if ( ! get_option( 'fictioneer_disable_splide' ) ) {
    $main_file_name .= '-splide';
  }

  // Application
  wp_enqueue_style(
    'fictioneer-application',
    get_template_directory_uri() . '/css/' . $main_file_name . '.css',
    [],
    $cache_bust
  );
...
function fictioneer_build_dynamic_scripts() {
  ...
  $file_path = Utils::get_cache_dir( 'build_dynamic_scripts' ) . 'dynamic-scripts.js';
...
function fictioneer_add_custom_scripts() {
  ...
  if ( ! get_option( 'fictioneer_bundle_scripts' ) ) {
    ...
    // Enqueue chapter
    if ( is_singular() && $post_type == 'fcn_chapter' && ! is_archive() && ! is_search() ) {
...
function fictioneer_headers_preload_font_links( $headers ) {
  ...
  $preload_links[] = trailingslashit( get_template_directory_uri() ) . 'assets/fontawesome/webfonts/fa-solid-900.woff2';
...
if ( ! function_exists( 'fictioneer_output_critical_fonts' ) ) {
  ...
  function fictioneer_output_critical_fonts() {
    ...
    if ( $critical_fonts_css ) {
      // Start HTML ---> ?>
      <style id="custom-critical-fonts" type="text/css" data-jetpack-boost="ignore" data-no-optimize="1" data-no-defer="1" data-no-minify="1"><?php
...
if ( ! function_exists( 'fictioneer_output_head_fonts' ) ) {
  ...
  function fictioneer_output_head_fonts() {
    ...
    // Setup
    $bundled_fonts = Utils::get_cache_dir( 'output_head_fonts' ) . 'bundled-fonts.css';
