...
function fictioneer_save_story_metaboxes( $post_id ) {
  ...
  if ( isset( $_POST['fictioneer_story_topwebfiction_link'] ) ) {
    $twf_url = Sanitizer::sanitize_url( $_POST['fictioneer_story_topwebfiction_link'], 'https://topwebfiction.com/' );
  ...
  if ( isset( $_POST['fictioneer_story_redirect_link'] ) && current_user_can( 'manage_options' ) ) {
    $redirect_url = Sanitizer::sanitize_url_https( $_POST['fictioneer_story_redirect_link'] );
  ...
  if (
    ...
    if ( $story_id ) {
      ...
      if ( $invalid_story && $current_story_id != $story_id ) {
        ...
        if ( get_option( 'fictioneer_enable_chapter_appending' ) ) {
          Utils_Admin::append_chapter_to_story( $post_id, $story_id );
        }
      }
    }
  ...
  if ( current_user_can( 'manage_options' ) || current_user_can( 'fcn_crosspost' ) ) {
    if ( isset( $_POST['fictioneer_chapter_story_override'] ) ) {
      ...
      if ( $story_id ) {
        ...
        if ( get_option( 'fictioneer_enable_chapter_appending' ) ) {
          Utils_Admin::append_chapter_to_story( $post_id, $story_id, true );
        }
...
function fictioneer_save_extra_metabox( $post_id ) {
  ...
  if ( isset( $_POST['fictioneer_landscape_image'] ) ) {
    $fields['fictioneer_landscape_image'] = Sanitizer::sanitize_image_id( $_POST['fictioneer_landscape_image'] );
  }

  // Custom page header
  if ( isset( $_POST['fictioneer_custom_header_image'] ) && current_user_can( 'fcn_custom_page_header', $post_id ) ) {
    $fields['fictioneer_custom_header_image'] = Sanitizer::sanitize_image_id( $_POST['fictioneer_custom_header_image'] );
  }
...
function fictioneer_save_support_links_metabox( $post_id ) {
  ...
  $pattern_map = array(
    'fictioneer_patreon_link' => '#^https://(www\.)?patreon\.com(?:/|$)#i',
    'fictioneer_kofi_link' => '#^https://(www\.)?ko-fi\.com(?:/|$)#i',
    'fictioneer_subscribestar_link' => '#^https://(www\.)?subscribestar\.com(?:/|$)#i',
    'fictioneer_paypal_link' => '#^https://(www\.)?(paypal\.com|paypal\.me)(?:/|$)#i'
  );

  foreach ( $pattern_map as $key => $pattern ) {
    if ( isset( $_POST[ $key ] ) ) {
      $fields[ $key ] = Sanitizer::sanitize_url( fictioneer_maybe_unslash( $_POST[ $key ] ), null, $pattern );
    }
  }

  if ( isset( $_POST['fictioneer_donation_link'] ) ) {
    $fields['fictioneer_donation_link'] = Sanitizer::sanitize_url_https(
      fictioneer_maybe_unslash( $_POST['fictioneer_donation_link'] )
    );
  }
...
function fictioneer_save_recommendation_metaboxes( $post_id ) {
  ...
  if ( isset( $_POST['fictioneer_recommendation_primary_url'] ) ) {
    $url = Sanitizer::sanitize_url_https( $_POST['fictioneer_recommendation_primary_url'] );
...
/**
 * Add SEO metabox to selected post types.
 *
 * @since 4.0.0
 *
...
function fictioneer_save_seo_metabox( $post_id, $post ) {
  ...
  if (
    ...
    in_array( $post->post_status, ['auto-draft'] )
  ) {
    return;
  }
...
if ( get_option( 'fictioneer_enable_seo' ) && ! fictioneer_seo_plugin_active() ) {
  add_action( 'save_post', 'fictioneer_save_seo_metabox', 10, 2 );
}
