...
**[Fictioneer 003 Admin Guard](https://github.com/Tetrakern/fictioneer/tree/main/mu-plugins)** enforces an allowlist of administrator user IDs defined via `define( 'FICTIONEER_ADMIN_ID_ALLOWLIST', [ ... ] )` in your `config.php`. If an unsanctioned user acquires administrator-like privileges, all of their capabilities are removed, their role is downgraded to _subscriber_, they are logged out, and the request is immediately terminated. This does not protect against other attack vectors such as arbitrary code execution, but it does prevent stealth administrator accounts.
